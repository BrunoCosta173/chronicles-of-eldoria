...
function spriteIcon(id, cls){ const sp=ITEMS[id]?.sprite; if(sp) return '<img class="item-sprite'+(cls?' '+cls:'')+'" src="'+sp+'" alt="'+esc(ITEMS[id].name)+'" draggable="false" loading="lazy" onerror="this.style.display=\'none\';this.nextSibling&&this.nextSibling.classList.remove(\'hidden\')"> <span class="hidden">'+esc(ITEMS[id].icon)+'</span>'; return esc(ITEMS[id]?.icon||''); }
